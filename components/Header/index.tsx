...
import { MenuDropdownRecord, MenuItemRecord, MenuQuery } from '@/graphql/generated';
                    ...
                    {menuData.map((menuItem, index) => (
                      <li key={menuItem.id} className="group relative">
                        {menuItem.path ? (
                          <Link
                            href={'' + menuItem.path}
                            className={`relative flex py-2 text-base text-dark group-hover:opacity-70 dark:text-white lg:mr-0 lg:inline-flex lg:px-0 lg:py-0`}
                          >
                            {menuItem.path === path && (
                              <motion.span
                                layoutId="underline"
                                className="absolute left-0 top-full w-full h-[1px] bg-dark"
                              ></motion.span>
                            )}
                            {menuItem.title}
                          </Link>
