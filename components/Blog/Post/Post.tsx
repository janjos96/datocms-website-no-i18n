...
import { Image as DatoImage, StructuredText, renderNodeRule } from 'react-datocms';
            ...
            <div>
              ...
              <div className="mb-10 flex items-center justify-between border-b border-body-color border-opacity-10 pb-4 dark:border-white dark:border-opacity-10">
                <div className="flex flex-col items-start md:flex-row md:items-center">
                  <Link
                    href={`/posts/author/${data.post.author.slug}`}
                  ...
                  >
                    <div className="mr-4">
                      <div className="relative h-10 w-10 overflow-hidden rounded-full">
                        <DatoImage
                          ...
                          data={data.post.author.picture.responsiveImage as ResponsiveImage}
                        />
                      </div>
                    </div>
                    <div className="w-full">
                      ...
                      <p className="text-xs text-body-color">{data.post.author.bio}</p>
                    </div>
                  </Link>
                ...
                <div className="mb-5">
                  <a
                    href={`/posts/tag/${data.post.tags[0].slug}`}
              ...
              <div>
                  ...
                  renderBlock={({ record }: any) => {
                    switch (record.__typename) {
                      ...
                      case 'NewsletterSubscriptionRecord':
                        const NewsletterSubscriptionRecord = record as NewsletterSubscriptionRecord;
                        return (
                          <NewsletterCTABlock
                            title={NewsletterSubscriptionRecord.title}
                            subtitle={NewsletterSubscriptionRecord.subtitle}
                            buttonLabel={NewsletterSubscriptionRecord.buttonLabel}
                          />
                        );
                      case 'CtaButtonWithImageRecord':
                        const CtaButtonWithImageRecord = record as CtaButtonWithImageRecord;
                        return (
                          <CTABlock
                ...
                <div className="mt-16 items-center justify-between sm:flex">
                  <div className="mb-5">
                    <h5 className="mb-3 text-sm font-medium text-body-color">Post Tags :</h5>
                    <div className="flex items-center">
                      {data.post.tags.map((tag) => {
                        return <TagButton key={tag.id} tag={tag.tag} slug={tag.slug} />;
                      })}
                    </div>
                  </div>
                  <div className="mb-5">
